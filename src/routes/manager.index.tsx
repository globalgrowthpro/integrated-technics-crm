...
import { employees, leads, activities, fmtMoney } from "@/lib/mock-data";
...
function ManagerDashboard() {
  ...
  const user = { name: "hafez Rahim", role: t("manager"), initials: "HR" };

  const totalLeads = storeLeads.length;
  const wonLeads = storeLeads.filter((l) => l.status === "won").length;
  const convRate = totalLeads ? Math.round((wonLeads / totalLeads) * 100) : 0;
  const todayActs = storeActivities.filter((a) => a.dueDate === new Date().toISOString().slice(0, 10));
  ...
  const recentActivities = useMemo(() =>
    [...storeActivities].sort((a, b) => (b.dueDate + b.time).localeCompare(a.dueDate + a.time)).slice(0, 6),
    [storeActivities]
  );
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-soft)]">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: dir === "rtl" ? "إجمالي العملاء" : "Total Leads", value: storeLeads.length, color: "text-sky-600" },
              { label: dir === "rtl" ? "عملاء مُغلقة" : "Won Deals", value: wonLeads, color: "text-emerald-600" },
              { label: dir === "rtl" ? "أنشطة معلقة" : "Pending Activities", value: storeActivities.filter((a) => a.status === "pending").length, color: "text-amber-600" },
            ].map((s) => (
