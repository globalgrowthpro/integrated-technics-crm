...
import { projects, employees, leads as mockLeads, fmtMoney } from "@/lib/mock-data";
...
function ProjectDetailsPage() {
  ...
  if (!project) {
    return (
      <AppShell panel={panel} user={user} pageTitle="Project">
        <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center">
          <p className="text-sm text-muted-foreground">Project <span className="font-mono">{projectId}</span> not found.</p>
          <Link to="/admin/projects" className="mt-3 inline-block text-sm font-semibold text-primary">Back to projects</Link>
        </div>
      </AppShell>
    );
  }
  ...
  const relatedLeads = mockLeads.filter((l) => l.company === project.client);
