...
import { Phone, Mail, Users2, MapPin, ClipboardCheck, RefreshCw, Plus, Check, Circle, Bookmark, Search, Menu, Clock4, List, CalendarDays, ChevronLeft, ChevronRight, Pencil, Trash2 } from "lucide-react";
...
function MyActivitiesPage() {
  ...
  useEffect(() => {
    ...
    setDateLabel(d.toLocaleDateString(undefined, { month: "short", day: "2-digit" }).toUpperCase().replace(" ", "."));
  }, []);

  const mine = useMemo(() => activities.filter((a) => a.owner === OWNER), [activities]);
  ...
  return (
    <AppShell panel="employee" user={{ name: OWNER, role: t("employee"), initials: "HR" }} pageTitle={t("myActivities")}>
      <div className="mx-auto w-full max-w-md md:max-w-6xl">
        <div className="rounded-3xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-primary to-orange-600 text-base font-bold text-primary-foreground shadow-[var(--shadow-brand)]">
              HR
            </div>
            <div className="min-w-0 flex-1">
              <div className="font-serif text-sm italic text-muted-foreground">Hi, lovable.</div>
              <div className="font-display text-2xl font-extrabold tracking-tight text-foreground">{dateLabel || "—"}</div>
            </div>
            <button className="flex h-9 w-9 items-center justify-center rounded-full text-primary hover:bg-primary/10" aria-label="Search"><Search className="h-4 w-4" /></button>
            <button className="flex h-9 w-9 items-center justify-center rounded-full text-primary hover:bg-primary/10" aria-label="Menu"><Menu className="h-4 w-4" /></button>
          </div>
          <div className="mt-4 flex items-center justify-between rounded-2xl bg-secondary/60 px-4 py-2.5">
