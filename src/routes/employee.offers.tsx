...
function EmployeeOffersPage() {
  ...
  const myQuotations = useMemo(() => quotations.filter((q) => q.owner === OWNER), [quotations]);
  ...
  return (
    <AppShell panel="employee" user={{ name: OWNER, role: t("employee"), initials: "HR" }} pageTitle={t("offers")}>
      <div className="mb-5 flex flex-wrap items-center gap-3">
        ...
        <button className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90 ms-auto">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((q) => (
                <tr key={q.id} className="transition hover:bg-primary/5">
                  <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{q.id}</td>
                  <td className="px-4 py-3 font-semibold text-foreground">{q.client}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${STATUS_COLORS[q.status] ?? "bg-secondary text-foreground"}`}>
                      {t(q.status as any) || q.status.replace("_", " ")}
                    </span>
                  </td>
